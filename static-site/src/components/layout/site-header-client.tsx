// Static showcase copy of site-header-client.tsx: identical, minus the sign-out server action
// (server actions are not supported in `output: "export"`). Never rendered with a user anyway.
"use client";

import Link from "next/link";
import { usePathname } from "next/navigation";
import { useEffect, useRef, useState } from "react";
import { Bookmark, ChevronDown, LayoutDashboard, Menu, Settings, Shield, X } from "lucide-react";
      ...
      {open && (
        <div role="menu" className="absolute right-0 mt-2 w-64 overflow-hidden rounded-2xl border border-ink-200 bg-white shadow-pop">
          ...
          <div className="p-1.5 text-sm">
            ...
            {/* Static showcase: no sessions, so no sign-out action. */}
          </div>
        </div>
      )}
    </div>
